...
void dfDisplacementMap::drawIntoMeWithAtFrom(int brushType, int x, int y, int diff_x, int diff_y){
	...
	char _r = CLAMP(-diff_x, 0, 32);
	char _g = 0;
	char _b = CLAMP(-diff_y, 0, 32);
	...
	for(int i = 0; i < width; i ++){
		for(int j = 0; j < height; j++){
			...
			switch (brushType){
				...
				default:
				if(dist < 32){
					unsigned char _newR = CLAMP(_rgbPixels[(j*width+i)*3 + 0] + ((_r + 30) - dist), 0, 255);
					unsigned char _newG = 0;
					unsigned char _newB = CLAMP(_rgbPixels[(j*width+i)*3 + 2] + ((_b + 30) - dist), 0, 255);
